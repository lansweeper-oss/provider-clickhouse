...
// Configure() call opens a new API session. However, upjet's Disconnect() is a
// no-op and never closes these sessions.
//
// This wrapper intercepts Configure() and returns a cached client when the
// configuration hasn't changed. The cache is keyed by a SHA-256 hash of the
// serialized config. This reduces session creation from once-per-reconcile to
// once-per-unique-config.
//
